...
local function OnSwitchInteraction()
    if not lightIsOn then
        ...
        World.FindObjectByName("Game Settings"):SetNetworkedCustomProperty("lightState", true)
    else
        switch:RotateTo(offRotation, 0.5, true)
        spawnedLight:Destroy()
        World.FindObjectByName("Game Settings"):SetNetworkedCustomProperty("lightState", false)
    end
